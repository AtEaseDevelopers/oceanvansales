...
class UpdateAgentRequest extends FormRequest
{
    ...
    {
        $id = Crypt::decrypt($this->route('agent'));
        $companyId = app()->bound('current_company_id') ? app('current_company_id') : null;
        $rules = [
            'employeeid' => ['nullable', 'string', 'max:20',
                Rule::unique('agents', 'employeeid')->where('company_id', $companyId)->ignore($id),
            ],
            'name' => 'required|string|max:255',
            'ic' => ['nullable', 'string', 'max:20',
                Rule::unique('agents', 'ic')->where('company_id', $companyId)->ignore($id),
            ],
