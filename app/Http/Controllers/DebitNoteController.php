...
class DebitNoteController extends Controller
{
    ...
    {
        $debitNotes = DebitNote::with('einvoices.invoice.customer')
    ...
    /**
     * Step 1: Note Type Selection (Debit Note)
     * Redirects directly to customer selection since note type is already determined
     */
    public function create()
    {
        return redirect()->route('debit-notes.select-customer', ['note_type' => 'debit']);
    }
    ...
    {
        // Get note_type from request (defaults to debit for debit note controller)
        $noteType = $request->input('note_type', 'debit');
        ...
        return view('debit_notes.select_customer', compact('customers', 'noteType'));
    }
    ...
    public function selectCurrency(Request $request)
    {
        ...
        return view('debit_notes.select_currency', compact('noteType', 'customerId'));
    }
    ...
    public function selectEinvoice(Request $request)
    {
        ...
        return view('debit_notes.select_einvoice', compact('noteType', 'customerId', 'currency', 'einvoices'));
    }

    /**
     * Step 5: Update E-Invoice (Enter debit note details)
     */
    public function updateEinvoice(Request $request)
    {
        ...
        return view('debit_notes.update_einvoice', compact('noteType', 'customerId', 'currency', 'einvoices'));
    }

    /**
     * Store and submit debit note (legacy method for web form)
     */
    ...
    /**
     * Submit debit note (new method following the pattern)
     */
    public function submitNote(Request $request)
    {
        ...
        $noteType = $request->input('note_type') ?? Session::get('note_type', 'debit');
        ...
        foreach ($einvoices as $einvoiceId => $invoice) {
            ...
            // Allow changes to equal the amount (full debit), with small tolerance for floating point
        ...
        try {
            $note = new DebitNote();
            $note->sku = (new DebitNote)->generateSku();
            ...
            foreach ($einvoices as $einvoiceId => $invoiceData) {
                $changesForXml[] = [
                    'sku' => $invoiceData['sku'] ?? '',
                    'description' => $invoiceData['description'] ?? 'Debit note adjustment',
            ...
            Log::info('Debit Note - TIN Verification Before XML Generation', [
                'debit_note_sku' => $note->sku,
        ...
        try {
            ...
            if ($response->successful()) {
                ...
                return response()->json([
                    ...
                    'redirect_url' => route('debit-notes.index'),
                ]);
            } else {
                return response()->json([
                    'success' => false,
    ...
    /**
     * Generate and save debit note PDF
     */
    protected function generateAndSaveNotePdf($note, $changes, $invoiceType)
    {
        try {
            ...
            // Generate PDF view
            $pdf = Pdf::loadView('debit_notes.print', [
                'debitNote' => $note,
                'changes' => $changes,
            ...
            Storage::put('public/lhdn/pdf/debit-note/' . $filename, $pdf->output());

            Log::info('Debit Note PDF Generated', [
                'debit_note_id' => $note->id,
                'sku' => $note->sku,
                'filename' => $filename,
            ]);
        } catch (\Exception $e) {
            Log::warning('Failed to generate debit note PDF', [
                'debit_note_id' => $note->id,
                'error' => $e->getMessage(),
            ]);
        }
    ...
    {
        $id = Crypt::decrypt($id);
        $debitNote = DebitNote::with('einvoices.invoice.customer')->findOrFail($id);

        return view('debit_notes.show', compact('debitNote'));
    }

    /**
     * Cancel a debit note (similar to e-invoice cancellation)
     */
    public function cancel($id, Request $request)
    {
        try {
            ...
            if (!$debitNote->uuid) {
                return response()->json([
                    'success' => false,
                    'message' => 'Debit note has not been submitted yet',
                ], 400);
            }
            ...
            Log::info('Debit Note Cancel - Starting', [
                'id' => $id,
                'uuid' => $debitNote->uuid,
                'current_status' => $debitNote->status,
                'reason' => $request->reason,
            ]);

            if (!$debitNote->submission_date) {
                return response()->json([
                    'success' => false,
            ...
            $hoursSinceSubmission = $debitNote->submission_date->diffInHours(now());
            ...
            $details = $this->myInvoisService->getDocumentDetails($debitNote->uuid, 1, 0);
            ...
            $result = $this->myInvoisService->cancelDocument($debitNote->uuid, $request->reason);

            Log::info('Debit Note Cancel - API Response', [
                'uuid' => $debitNote->uuid,
                'result' => $result,
                'result_keys' => is_array($result) ? array_keys($result) : 'not_array',
            ]);

            $status = DebitNote::STATUS_INVALID;
            if (is_array($result)) {
                $status = $result['status'] ?? $result['documentStatus'] ?? DebitNote::STATUS_INVALID;
            }
            ...
            Log::info('Debit Note Cancel - Success', [
                'id' => $debitNote->id,
                'uuid' => $debitNote->uuid,
                'new_status' => $status,
            ]);
            ...
            Log::error('Debit Note Cancel - Validation Error', [
                'errors' => $e->errors(),
            ]);
            ...
            Log::error('Debit Note Cancel - Error', [
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString(),
            ]);
            ...
            return response()->json([
                'success' => false,
                'message' => 'Failed to cancel debit note: ' . $e->getMessage(),
            ], 500);
        }
    }
}
