...
@section('content')
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active">Step 5: Enter Credit Note Details</li>
    </ol>
    ...
    $('#creditNoteForm').on('submit', function(e) {
        ...
        $.ajax({
            ...
            success: function(response) {
                HideLoad();
                if (response.success) {
                    noti('s', 'Success', 'Credit note submitted successfully!');
                    setTimeout(function() {
            ...
            error: function(xhr) {
                HideLoad();
                const message = xhr.responseJSON?.message || 'Failed to submit credit note';
